...
# create a class named GetInfoGuest
class GetInfoGuest:
    # dunder method
    ...
    def create_widgets(self,screen):
          ...
          self.Message1.place(relx=0.03, rely=0.11, relheight=0.84, relwidth=0.5)
          ...
          self.Message2.place(relx=0.45, rely=0.19, relheight=0.74, relwidth=0.07)
          ...
          self.Message3.place(relx=0.50, rely=0.11, relheight=0.79, relwidth=0.45)
          self.Message3.configure(background="#ffffff")
          self.Message3.configure(text='''GET INFO OF ANY GUEST''', font=("Arial", 15,"bold"))
          ...
          self.Button1.configure(text='''SUBMIT''',font=("Arial",10,"bold"))
...
class SubmitGuestInfo(GetInfoGuest):
    # member method for submit data      
    def submit(self):
        # get value
        self.a = self.Entry1.get()
        # create Database class object database_ope
        database_ope = Database()
        # through database_ope obj call get_guest_details() function 
        __res = database_ope.get_guest_details(self.a)
        if __res:
             for val in __res:   
               self.Text1.insert(END, f"{val}\n")
        else:
               self.Text1.insert(END, "Invalid room number.")

# create a Tk() class object          
root = Tk()
submit_info = SubmitGuestInfo(root)
